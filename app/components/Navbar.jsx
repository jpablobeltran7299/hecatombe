...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <div className="hidden md:flex items-center gap-6">
          {LINKS.map(({ href, label }) => (
            <Link key={href} href={href}
              className="text-white text-xs font-bold uppercase tracking-widest hover:text-orange-500 transition">
              {label}
            </Link>
          ))}
          ...
          <form onSubmit={handleSearch} className="flex items-center gap-2">
            {searchOpen && (
              <input
                ...
                placeholder="Buscar..."
                className="bg-white/10 border border-white/30 focus:border-orange-500 text-white text-xs px-3 py-1.5 rounded-lg outline-none placeholder:text-white/50 transition w-36"
              />
            )}
            <button
          ...
          {/* Ícono carrito */}
          <Link href="/carrito" className="relative text-white hover:text-orange-500 transition">
          ...
          {esAdmin && (
            <Link href="/admin"
              className="border border-white/40 hover:border-orange-500 text-white hover:text-orange-500 text-xs font-black uppercase px-3 py-2 rounded-lg transition">
              Admin
            </Link>
          )}
          ...
          {user ? (
            ...
            <Link href="/login"
              className="border border-white text-white hover:bg-white hover:text-black text-xs font-black uppercase px-4 py-2 rounded-lg transition">
              Iniciar sesión
            </Link>
          )}
        </div>
      ...
      {/* Buscador mobile */}
      {searchOpen && (
        <div className="md:hidden bg-black border-b border-white/20 px-4 py-3">
          <form onSubmit={handleSearch} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-white/10 border border-white/30 focus:border-orange-500 text-white text-sm px-3 py-2 rounded-lg outline-none placeholder:text-white/50 transition"
            />
            <button type="submit"
        ...
        <div className="md:hidden bg-black border-b-2 border-orange-500 px-4 py-4 flex flex-col gap-4 sticky top-[53px] z-40">
          {LINKS.map(({ href, label }) => (
            <Link key={href} href={href}
              ...
              className="text-white text-sm font-bold uppercase tracking-widest hover:text-orange-500 transition border-b border-white/20 pb-3 last:border-0 last:pb-0">
              {label}
            </Link>
          ))}
          {esAdmin && (
            <Link href="/admin"
              onClick={() => setMenuOpen(false)}
              className="text-white text-sm font-black uppercase tracking-widest hover:text-orange-500 transition border-b border-white/20 pb-3">
              Admin
            </Link>
          )}
