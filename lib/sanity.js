...
export async function getProductosDestacados() {
  return client.fetch(`
    *[_type == "producto" && destacado == true && activo != false] | order(_createdAt desc) {
...
export async function getProducto(id) {
  return client.fetch(`*[_type == "producto" && _id == $id][0]{
    ...
    "marca": marca->nombre,
    "categoria": categoria->nombre,
    imagenes, mlUrl, tipo, fechaEstimada
  }`, { id })
}
...
export async function getDinamicas() {
  return client.fetch(`
    *[_type == "dinamica"] | order(_createdAt desc) {
      ...
      enlace, activa, numerosTotal, numerosVendidos
    }
  `)
}
...
export async function getTematicas() {
  return client.fetch(`*[_type == "tematica"] | order(orden asc){ _id, nombre }`)
}

export async function getLineas() {
  return client.fetch(`*[_type == "linea"] | order(orden asc){ _id, nombre }`)
}
export async function getUniversos() {
  return client.fetch(`*[_type == "universo"] | order(orden asc){ _id, nombre }`)
}
